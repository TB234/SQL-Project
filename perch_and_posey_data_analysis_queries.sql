...
/* Write a query that displays all the data in the occurred_at, account_id, and channel columns of 
...
--Use the accounts table to find all the companies whose names does not start with 'C'.
SELECT name 
FROM accounts
WHERE name NOT LIKE 'C%' ;

--Use the accounts table to find all companies whose names does not contain the string 'one' somewhere in the name.
SELECT name 
FROM accounts
WHERE name NOT LIKE '%one%' ;

--Use the accounts table to find all companies whose names does not end with 's'.
SELECT name 
FROM accounts
...
--Which account (by name) placed the earliest order? Include the account name and the date of the order.
...
--Use DISTINCT to test if there are any accounts associated with more than one region. Yes, both queries give same result
SELECT COUNT(*)
FROM accounts a
JOIN sales_reps s
ON s.id = a.sales_rep_id
...
--Have any sales reps worked on more than one account? Yes, 1st query gives 351 while 2nd gives 50
SELECT  COUNT(*)
FROM accounts a
JOIN sales_reps s
ON s.id = a.sales_rep_id;
...
--Which account has the most orders? There are 3 accounts with the topmost 21 orders
...
--In which month of which year did Walmart spend the most on gloss paper in terms of dollars? May, 2016
...
--What is the average amount of standard paper sold on the first month that any order was placed in the orders table (in terms of quantity).
...
--What is the average amount of gloss paper sold on the first month that any order was placed in the orders table (in terms of quantity).
...
FROM (SELECT o.id order_id, r.name region
      FROM sales_reps s
      JOIN region r
      ON r.id = s.region_id
      JOIN accounts a
      ON s.id = a.sales_rep_id
      JOIN orders o
      ON a.id = o.account_id)tab3

WHERE region =  
      (SELECT region
      FROM (SELECT r.name region,  SUM(total_amt_usd) total_usd 
            FROM sales_reps s
            JOIN region r
            ON r.id = s.region_id
            JOIN accounts a
            ...
            ON a.id = o.account_id
            GROUP BY  r.name 
            ORDER BY region) tab2

      WHERE tab2.total_usd =        
...
--Using the accounts table, create two groups: one group of company names that start with a number and a second group of those company names that start with a letter.
...
--Create first and last name columns that hold the first and last name for every rep name in the sales_reps table.
...
-- Show primary point of contact for each company (first and last name), their company, email address and initial assigned password
...
--Write a query to extract the date from the sf_crime_data dataset
...
-- Create running total of standard_amt_usd over ordered time
SELECT standard_amt_usd,
      SUM(standard_amt_usd) OVER(occurred_at ORDER BY date) AS running_total
FROM orders;

-- Create running total of standard_amt_usd over ordered time and partitioned over the years
...
-- Ordered by year and partitioned for each account_id show a running SUM, COUNT, AVG, MIN and MAX of total_amt_usd
SELECT id,
       account_id,
...
--Divide the accounts into 4 levels in terms of the amount of standard_qty for their orders. Your resulting table should have the account_id, 
...
--Divide the accounts into two levels in terms of the amount of gloss_qty for their orders. Your resulting table should have the account_id, 
...
--Divide the orders for each account into 100 levels in terms of the amount of total_amt_usd for their orders. Your resulting table should have the account_id,
...
---You want to see each account who has a sales rep and each sales rep that has an account, but also each account that does not have a sales rep and 
...
-- SHOW web events that occur not more than 1 day for each other for each account and show the channel for each event
...
--show accounts that made multiple orders within 30 days  
...
-- Write a query that uses UNION ALL on two instances (and selecting all columns) of the accounts table to show Walmart and Disney accounts only
SELECT * 
FROM accounts a
WHERE name = 'Walmart'
UNION ALL
...
--Write a query that uses UNION ALL on two instancesof the accounts table in a common table expression and name it double_accounts. 
